/**
 * Tests for Server Stop API Route
 */
...
describe('POST /api/server/stop', () => {
  ...
  test('認証済みユーザーがサーバーを停止できる', async () => {
    const mockUser = {
      id: 'user-123',
    ...
    vi.mocked(ec2Service.stopInstance).mockResolvedValue(undefined);
    ...
    const request = new NextRequest('http://localhost:3000/api/server/stop', {
      method: 'POST',
    });
    ...
    expect(concurrencyService.acquireOperationLock).toHaveBeenCalledWith('stop');
    expect(ec2Service.stopInstance).toHaveBeenCalledWith('i-1234567890abcdef0');
  ...
  test('pending状態でstop操作を試みると400エラーが返される', async () => {
    const mockUser = {
      id: 'user-123',
      line_sub: 'test-line-sub',
      display_name: 'Test User',
    ...
    vi.mocked(concurrencyService.cleanupExpiredLocks).mockResolvedValue(undefined);
    vi.mocked(concurrencyService.acquireOperationLock).mockResolvedValue('lock-123');
    vi.mocked(ec2Service.stopInstance).mockRejectedValue(
      new Error('Cannot stop instance while it is still starting')
    );
    vi.mocked(concurrencyService.releaseOperationLock).mockResolvedValue(undefined);
    ...
    expect(data.error.message).toBe('現在の状態ではこの操作を実行できません');
    expect(concurrencyService.releaseOperationLock).toHaveBeenCalledWith('lock-123');
  });
});
